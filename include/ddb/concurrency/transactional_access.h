...
class TransactionalTableHeap final {
 public:
  ...
  [[nodiscard]] std::optional<ddb::execution::RecordId> insert(Transaction& tx,const ddb::execution::Tuple& tuple){if(!locks_.lock_exclusive(tx,ResourceId(heap_.metadata_page_id())))return{};return heap_.insert(tx,tuple);}
  [[nodiscard]] bool erase(Transaction& tx,ddb::execution::RecordId id){return locks_.lock_exclusive(tx,ResourceId(id.page_id))&&heap_.erase(tx,id);}
...
class TransactionalBPlusTree final {
 public:
  ...
  [[nodiscard]] bool insert(Transaction& tx,ddb::index::KeyType key,ddb::index::RecordId value){return locks_.lock_exclusive(tx,ResourceId(tree_.metadata_page_id()))&&tree_.insert(tx,key,value);}
  [[nodiscard]] bool remove(Transaction& tx,ddb::index::KeyType key){return locks_.lock_exclusive(tx,ResourceId(tree_.metadata_page_id()))&&tree_.remove(tx,key);}
