...
void corrupt_header_field(std::size_t offset,std::byte value){auto p=path("ddb_page_corrupt.db");{PageManager m(p);const auto id=m.allocate_page();EXPECT(id==PageId(0));m.flush();}{std::fstream f(p,std::ios::binary|std::ios::in|std::ios::out);f.seekp(static_cast<std::streamoff>(4096+offset));const char c=static_cast<char>(std::to_integer<unsigned char>(value));f.write(&c,1);}try{PageManager m(p);Page x;m.read_page(PageId(0),x);EXPECT(false);}catch(const StorageError&){EXPECT(true);}std::error_code e;std::filesystem::remove(p,e);}
